...
def SimpleSolver(u,v,p,u_star,v_star,u_new,v_new,p_new,d_y,d_x,N,alpha_p,mu,b,h, Re):
    ...
    while it<itermax and (err_u > tol or err_v > tol or err_p > tol):
        ...
        tmp_x=A@u_star-b
        norm_res_x = np.linalg.norm(tmp_x) / np.linalg.norm(b)
        res_x.append(np.linalg.norm(norm_res_x))
        ...
        tmp_y=A@v_star-b
        norm_res_y = np.linalg.norm(tmp_y) / np.linalg.norm(b)
        res_y.append(np.linalg.norm(norm_res_y))
        ...
        # Continuity increment as error measure
    ...
    # plt.plot(res_x, label='Residual X-Momentum')
    # plt.plot(res_y, label='Residual Y-Momentum')
    # plt.yscale('log')
    ...
    # plt.grid(True)
    # plt.show()
    print(f"Converged in {it} iterations with error {err_u:.2e} {err_v:.2e} {err_p:.2e} with N={N} and Re={Re}")
    return u,v,p
...
def smooth_curve(x, y, smooth_factor=0.5, num_points=200):
    spline = UnivariateSpline(x, y, s=smooth_factor)  # Crea la spline
    x_smooth = np.linspace(x.min(), x.max(), num_points)  # Nuovi punti x
    y_smooth = spline(x_smooth)  # Valori interpolati
